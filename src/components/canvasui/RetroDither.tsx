...
/**
 * Canvas UI — Retro Dither. Vendored from @canvas-ui/retro-dither-react.
 *
 * Two deviations from the published source, both type-only:
 *   • `React.CSSProperties` → `CSSProperties` imported as a type (the UMD React
 *     namespace isn't in scope in a module under `jsx: react-jsx`).
 *   • the `../rect-cache` helper is vendored alongside at src/components/rect-cache.ts.
 *
 * Requires the experimental html-in-canvas APIs (`drawElementImage` /
 * `requestPaint` / `layoutsubtree`). Where they're missing, `supportsHtmlInCanvas()`
 * is false, the children render normally in flow and the WebGL pass composites
 * fully transparent — a graceful no-op rather than a broken box.
 */
